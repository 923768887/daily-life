...
// 从 API 获取纪念日
const { data: anniversaryData } = await useAuthFetch<ApiResponse<PaginatedData<any>>>('/api/anniversary/list')
...
// 从 API 获取日记
const { data: diaryData } = await useAuthFetch<ApiResponse<PaginatedData<any>>>('/api/diary/page', {
  method: 'POST',
  body: { page: 1, size: 3 },
})
...
// 从 API 获取每日情话
const { data: quoteData } = await useFetch('/api/lovequote/daily')
  ...
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-4 gap-3">
      <NuxtLink 
        ...
        :key="action.path"
        :to="action.path"
        class="flex flex-col items-center gap-2 p-4 rounded-xl bg-white shadow-sm hover:shadow-md transition-shadow"
      >
